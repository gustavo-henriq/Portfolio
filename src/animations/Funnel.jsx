...
export function Funnel({language}){
 ...
 const root=ref.current,scene=conditions.desktop?root.closest('.project-spread'):root,slips=[...root.querySelectorAll('.listing-slip')],bubble=root.querySelector('.match-bubble');
 const tl=gsap.timeline({scrollTrigger:{trigger:scene,start:()=>conditions.mobile?'top 94px':'top 112px',end:()=>`+=${window.innerHeight*config.funnelScrollVh/100}`,pin:scene,pinSpacing:true,anticipatePin:1,scrub:.45,markers:debugMarkers(),invalidateOnRefresh:true}});
 ...
 tl.fromTo(bubble,{scale:.75,opacity:0},{scale:1,opacity:1,transformOrigin:'center',duration:.2},.72);
